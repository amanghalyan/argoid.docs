...
#export SPARK_SUBMIT_OPTIONS="--queue ds_bangalore --driver-cores 1 --driver-memory 2G --executor-cores 3 --num-executors 3 --executor-memory 3G"
export ZEPPELIN_JAVA_OPTS="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=30023 -Dcom.sun.management.jmxremote.local.only=false -Dcom.sun.management.jmxremote.authenticate=false -javaagent:{{jmx_exporter_java_agent_file}}={{ zeppelin_jmx_port }}:{{ jmx_exporter_config_file}} -Dcom.sun.management.jmxremote.ssl=false"
...
# export ZEPPELIN_MEM            		
# Zeppelin jvm mem options Default -Xms1024m -Xmx1024m -XX:MaxPermSize=512m

export ZEPPELIN_MEM="-Xms{{zeppelin_jvm_heap_size}}m -Xmx{{zeppelin_jvm_heap_size}}m -XX:MaxPermSize=512m"

# export ZEPPELIN_INTP_MEM       		
# zeppelin interpreter process jvm mem options. Default -Xms1024m -Xmx1024m -XX:MaxPermSize=512m
# export ZEPPELIN_INTP_JAVA_OPTS 		
# zeppelin interpreter process jvm options.
# export ZEPPELIN_SSL_PORT       		
# ssl port (used when ssl environment variable is set to true)
# export ZEPPELIN_JMX_ENABLE    		
# Enable JMX feature by defining "true"
# export ZEPPELIN_JMX_PORT 
# Port number which JMX uses. Default: "9996"
# export ZEPPELIN_LOG_DIR        	
# Where log files are stored.  PWD by default.
# export ZEPPELIN_PID_DIR        		
# The pid files are stored. ${ZEPPELIN_HOME}/run by default.
# export ZEPPELIN_WAR_TEMPDIR    		
# The location of jetty temporary directory.
...
# export ZEPPELIN_NOTEBOOK_S3_ENDPOINT      
...
# export ZEPPELIN_NOTEBOOK_GCS_STORAGE_DIR     
# GCS "directory" (prefix) under which notebooks are saved. E.g. gs://example-bucket/path/to/dir
# export GOOGLE_APPLICATION_CREDENTIALS         
# Provide a service account key file for GCS and BigQuery API calls (overrides application default credentials)
# export ZEPPELIN_NOTEBOOK_MONGO_URI				
# MongoDB connection URI used to connect to a MongoDB database server. Default "mongodb://localhost"
# export ZEPPELIN_NOTEBOOK_MONGO_DATABASE		
# Database name to store notebook. Default "zeppelin"
# export ZEPPELIN_NOTEBOOK_MONGO_COLLECTION 
# Collection name to store notebook. Default "notes"
# export ZEPPELIN_NOTEBOOK_MONGO_AUTOIMPORT	
# If "true" import local notes under ZEPPELIN_NOTEBOOK_DIR on startup. Default "false"
...
# export ZEPPELIN_INTERPRETER_LOCALREPO         
# Local repository for interpreter's additional dependency loading
# export ZEPPELIN_INTERPRETER_DEP_MVNREPO       
...
# export ZEPPELIN_HELIUM_NPM_INSTALLER_URL      
# Remote Npm installer url for Helium dependency loader
# export ZEPPELIN_HELIUM_YARNPKG_INSTALLER_URL  
# Remote Yarn package installer url for Helium dependency loader
# export ZEPPELIN_NOTEBOOK_STORAGE 		
# Refers to pluggable notebook storage class, can have two classes simultaneously with a sync between them (e.g. local and remote).
...
# export ZEPPELIN_IMPERSONATE_CMD       
# Optional, when user want to run interpreter as end web user. eg) 'sudo -H -u ${ZEPPELIN_IMPERSONATE_USER} bash -c '
